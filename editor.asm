...
.var OP2_Stack_PTR 1
.var OP2_Stack     STACK_SIZE
.var TEMP_STACK     STACK_SIZE
.var RSEED         2
...
_RESET
    ...
    lda MEMTOP.lo; sta <BufTop+0>; sta <BufAfter+0>
    lda MEMTOP.hi; sta <BufTop+1>; sta <BufAfter+1>
    lda 0; tay; sta [<BufAfter>+Y]
    lda 0; tay; sta [<BufCursor>+Y]
    
_RESETWARM
    lda $00; sta <BufBottom+0>; lda $03; sta <BufBottom+1>
    lda MEMTOP.lo; sta <BufTop+0>; lda MEMTOP.hi; sta <BufTop+1>
    # Silence Audio
    ...
    sec
    lda <BufTop+0>; sbc 1; sbc <BufBottom+0>; sta <R7+0>; sta <BufSize+0>
...
_MAIN
    ...
    pha
    lda <BufCursor+0>; inc A; cmp <BufAfter+0>; bne (canwrite)
    lda <BufCursor+1>; cmp <BufAfter+1>; bne (canwrite)
        pla
        jmp [noinput]
    __canwrite
    pla
...
_DrawScreen
    ...
    lda $00; sta <dR0+0>
    lda $68; sta <dR0+1>
    lda <PagePTR+0>; sta <dR1+0>
    lda <PagePTR+1>; sta <dR1+1>
    ldy 0
    
    lda <BufCursor+0>; sta <TempBufCursor+0>
    lda <BufCursor+1>; sta <TempBufCursor+1>
    lda <BufAfter+0>; sta <TempBufAfter+0>
    lda <BufAfter+1>; sta <TempBufAfter+1>
    
    cli
    # 
    
    __PreCursor
        lda <dR1+0>; cmp <TempBufCursor+0>; bne (notsame)
        lda <dR1+1>; cmp <TempBufCursor+1>; bne (notsame)
            bra (break)
        ___notsame
        
        lda [<dR1>+Y]; bne (notend)
            jmp [GoEND]
        ____notend
        cmp LF; bne (continue)
            jsr [HandleLF]
    ...
    lda <TempBufAfter+0>; sta <dR1+0>
    lda <TempBufAfter+1>; sta <dR1+1>
    __PostCursor
        lda <dR1+0>; cmp <BufTop+0>; bne (notsame)
        lda <dR1+1>; cmp <BufTop+1>; bne (notsame)
            bra (GoEND)
        ___notsame
        ...
        ____return
        clc
        lda <dR1+0>; adc 1; sta <dR1+0>
        lda <dR1+1>; adc 0; sta <dR1+1>
    ...
    lda <BufCursor>; cmp <TempBufCursor>; bne (repeat)
    lda <BufAfter>; cmp <TempBufAfter>; bne (repeat)
rts
    ___repeat
jmp [DrawScreen]
